...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
	...
	resolveWebviewView(webviewView: vscode.WebviewView) {
		...
		webviewView.webview.onDidReceiveMessage(async (message) => {
			if (message.type === 'saveAgentSeats') {
				...
				// Automatically spawn an agent for Antigravity if none exist
				if (this.agents.size === 0) {
					console.log('[Pixel Agents] Auto-spawning first agent for Antigravity');
					launchNewTerminal(
						this.nextAgentId, this.nextTerminalIndex,
						this.agents, this.activeAgentId,
						this.webview, this.persistAgents,
	...
	dispose() {
		...
		for (const id of [...this.agents.keys()]) {
			removeAgent(id, this.agents, this.persistAgents);
		}
	}
}
